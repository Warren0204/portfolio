...
- D31 The identity line is one row at every width on all three cards, and one component, `identity-line`, rather than a copy in each of two section files. Supersedes D20, whose two-row block below 440 is gone. D18 removed the numeral prefix that caused the original 20px stub at 412, so the one-row form fits there with a real line and the two-row rule had stopped solving anything; what it did instead was make the project cards read as broken next to the Experience card directly below them, which kept chip, rule and date on one row at the same width. The row is `flex-wrap: nowrap` with the chip and the date at `flex: none` and `white-space: nowrap`, so the date can never stack under the chip and the rule gives ground first; the rule's old `min-width: 20px` is `0` for the same reason, because a floor on the only item free to shrink widens the row past the card instead of holding a minimum, and the card's `clip-path` then eats the date. At 360 and under, one compact step keeps the longest pair ("Capstone project" with "2025 to 2026") on its row: `--text-eyebrow-compact` at 10px, 0.12em tracking, 4px 8px on the chip, 8px gaps, applied to the Experience card as well because a card that held its size while the two above it shrank would break the very thing the component exists for. That 10px is the only value allowed below the 11px floor stated in typography.css, and the floor comment now says so. At 320 even the compact recipe leaves the longest pair about 12px of rule, so the rule is hidden there and the chip and the date hold the two ends on their own; the auto margin on the date, which was on the Projects copy alone and was doing nothing while the rule was present, is what keeps it flush right.
- D32 The AI layer chip reads "Claude Sonnet (photo verification)". "Vision" was doing the work of the parenthesis beside it: photo verification is vision, said in the reader's words rather than the vendor's.
...
| ------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Remove scroll cue`                         | The SCROLL label, its rule and the cue-slide keyframe go; the hero closes on the stats row and its bottom spacing is the section padding alone                                                                                        |
| `Restore availability pills`                | The header chip and the Contact card are back where they were, with their original text and visibility, reversing the hero eyebrow line; only the dots stay gone                                                                      |
| `Restore contact action`                    | Get in touch is back as the third hero action in the ghost variant, reversing `Trim hero actions`; the row is one per row below 560 and one row of three above it                                                                     |
| `Trim about copy`                           | The About paragraph ends at the Anthropic API sentence; the certifications keep their own cards and the target roles stay in the Contact paragraph                                                                                    |
| `Trim hero actions`                         | The hero offers View projects and View CV; the jump to the contact form goes, since the nav, the phone tab bar and the end of the scroll all already reach it                                                                         |
| `Reduce hero motion`                        | The hero line types once and its caret settles instead of blinking forever, and the loading screen resolves as it splits rather than 920ms later; the hold, delete and restart timings retire with the loop                           |
| `Remove dead tokens`                        | `--accent-dot`, `--accent-glow`, `--text-body-lg` and `--z-zoom` had no consumer anywhere in css/, js/ or index.html                                                                                                                  |
| `Fix credentials duplicates`                | The capstone team size is stated once, in the bullet that carries the sponsor with it, and track 02 names the SQL Associate by id instead of re-stating its title, issuer and status                                                  |
| `Fix layout alignment`                      | A dead 14px lead margin, a duplicated submit height, both project shells through createCard, the tree's unreachable reset, and `min-width: 0` on the contact grid so the hCaptcha iframe stops widening the page by two pixels at 320 |
| `Rewrite about copy`                        | The About paragraph says what I build, where I built it, and what I am looking for; the repeated counts, location and reply promise go from every place but their keeper                                                              |
